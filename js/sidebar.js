(function(){
    ...
    function render() {
        ...
        h += '<span class="logo-text">家計簿</span>';
        h += '<span class="logo-sub">KeuanganKu</span>';
        h += '</div>';
        h += '</div>';
        ...
        h += '<span class="menu-text">ダッシュボード</span>';
        h += '<span class="menu-sub">Dashboard</span>';
        h += '</a></li>';
        h += '<li><a href="transaksi.html" class="' + (cp === 'transaksi.html' ? 'active' : '') + '">';
        h += '<span class="menu-icon">💳</span>';
        h += '<span class="menu-text">取引</span>';
        h += '<span class="menu-sub">Transaksi</span>';
        h += '</a></li>';
        h += '<li><a href="budget.html" class="' + (cp === 'budget.html' ? 'active' : '') + '">';
        h += '<span class="menu-icon">💰</span>';
        h += '<span class="menu-text">予算</span>';
        h += '<span class="menu-sub">Budget</span>';
        h += '</a></li>';
        h += '<li><a href="tracking.html" class="' + (cp === 'tracking.html' ? 'active' : '') + '">';
        h += '<span class="menu-icon">📈</span>';
        h += '<span class="menu-text">追跡</span>';
        h += '<span class="menu-sub">Tracking</span>';
        h += '</a></li>';
        h += '<li><a href="hapus_data_lama.html" class="' + (cp === 'hapus_data_lama.html' ? 'active' : '') + '">';
        h += '<span class="menu-icon">🧹</span>';
        h += '<span class="menu-text">ツール</span>';
        h += '<span class="menu-sub">Tools</span>';
        h += '</a></li>';
        h += '</ul>';
        h += '</nav>';
        h += '<div class="sidebar-footer">';
        h += '<div class="footer-copyright">© 2026 家計簿</div>';
        h += '<div class="footer-version">v9.0.0</div>';
        h += '</div>';
        h += '</aside>';
    ...
    function updateSaldo() {
        ...
        try {
            ...
            elS.innerHTML = '💰 残高 (Saldo)<br><small>¥' + saldo.toLocaleString('ja-JP') + ' ➡️ ' + rupiahFull + ' (' + rupiahCompact + ')</small>';
    ...
    function updateHutangHeader() {
        ...
        try {
            var hutang = Storage.getData('keuangan_hutang');
            var trx = Storage.getData('keuangan_transaksi');
            var totalBayar = 0;
            for (var i = 0; i < trx.length; i++) {
                var it = trx[i];
                if (it.jenis === 'pengeluaran' && it.kategori && it.kategori.includes('借金返済')) {
                    totalBayar += safeNumber(it.total);
                }
            }
            var totalHutang = 0;
            for (var i = 0; i < hutang.length; i++) {
                totalHutang += safeNumber(hutang[i].jumlah);
            }
            ...
            elH.innerHTML = '💳 借金 (Hutang)<br><small>¥' + sisa.toLocaleString('ja-JP') + ' ➡️ ' + rupiahFull + ' (' + rupiahCompact + ')</small>';
